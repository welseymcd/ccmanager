...
export class SessionManager extends EventEmitter {
  ...
  private async reattachTmuxSession(sessionId: string): Promise<pty.IPty | null> {
    ...
    try {
      ...
      const onDataHandler = (data: string) => {
        logger.info(`[SessionManager] ====== PTY DATA RECEIVED (reattached) ======`);
        logger.info(`[SessionManager] Session: ${sessionId}`);
        logger.info(`[SessionManager] Data length: ${data.length}`);
        logger.info(`[SessionManager] First 50 chars: ${JSON.stringify(data.substring(0, 50))}`);
        ...
        session.state = session.promptDetector.processOutput(filteredData);
        
        // Emit state change event if state changed
        if (previousState !== session.state) {
          logger.info(`Session ${sessionId} state changed from ${previousState} to ${session.state}`);
          this.emit('sessionStateChanged', { sessionId, state: session.state, previousState });
        }
        
        this.appendToBuffer(session, filteredData);
        session.lastActivity = new Date();
        logger.info(`[SessionManager] Emitting sessionData event for reattached session ${sessionId}, data length: ${filteredData.length}`);
      ...
      const hasHandlers = ptyProcess && (ptyProcess as any)._events && (ptyProcess as any)._events.data;
      logger.info(`[SessionManager] Reattached PTY has onData handlers: ${hasHandlers}`);
      ...
      // Double-check handlers are working by sending a test command after a delay
      setTimeout(() => {
        logger.info(`[SessionManager] Testing reattached session ${sessionId} handlers...`);
        ptyProcess.write('\x0c'); // Send Ctrl+L to refresh screen
      }, 300);
      
      // Send a refresh command to ensure tmux sends current screen content
      // Use tmux capture-pane to force current buffer update
      setTimeout(async () => {
        try {
          logger.debug(`Capturing tmux buffer for session ${tmuxName}`);
          const currentBuffer = await this.getTmuxBuffer(sessionId, 100);
          if (currentBuffer && currentBuffer.trim()) {
            logger.debug(`Sending current buffer (${currentBuffer.length} bytes) to frontend`);
            // Emit the current buffer as output
            this.emit('sessionData', { sessionId, data: currentBuffer });
          }
        } catch (err) {
    ...
    try {
      ...
      if (this.useTmux) {
        ...
        try {
          ...
          // Capture initial tmux buffer after session creation
          setTimeout(async () => {
            try {
              logger.debug(`Capturing initial tmux buffer for new session ${tmuxName}`);
              const currentBuffer = await this.getTmuxBuffer(sessionId, 100);
              if (currentBuffer && currentBuffer.trim()) {
                logger.debug(`Sending initial buffer (${currentBuffer.length} bytes) to frontend`);
                // Emit the current buffer as output
                this.emit('sessionData', { sessionId, data: currentBuffer });
              }
            } catch (err) {
              logger.debug(`Could not capture initial tmux buffer: ${err}`);
            }
          }, 500);
        } catch (error: any) {
      ...
      ptyProcess.onData((data: string) => {
        logger.info(`[SessionManager] ====== PTY DATA RECEIVED ======`);
        logger.info(`[SessionManager] Session: ${sessionId}`);
        logger.info(`[SessionManager] Data length: ${data.length}`);
        logger.info(`[SessionManager] First 50 chars: ${JSON.stringify(data.substring(0, 50))}`);
        logger.info(`[SessionManager] ===============================`);
        ...
        this.appendToBuffer(session, filteredData);
        onData(filteredData);
        logger.info(`[SessionManager] Emitting sessionData event for session ${sessionId}, data length: ${filteredData.length}`);
        this.emit('sessionData', { sessionId, data: filteredData });
      ...
      // Verify PTY handlers are set up
      const hasHandlers = ptyProcess && (ptyProcess as any)._events && (ptyProcess as any)._events.data;
      logger.info(`[SessionManager] After setup - PTY has onData handlers: ${hasHandlers}`);
      logger.info(`[SessionManager] PTY event listeners: ${JSON.stringify(Object.keys((ptyProcess as any)._events || {}))}`);
  ...
  async writeToSession(sessionId: string, data: string): Promise<void> {
    logger.info(`[SessionManager] ====== WRITE TO SESSION ======`);
    logger.info(`[SessionManager] Session ID: ${sessionId}`);
    logger.info(`[SessionManager] Data: ${JSON.stringify(data)}`);
    logger.info(`[SessionManager] Data (hex): ${Buffer.from(data).toString('hex')}`);
    logger.info(`[SessionManager] Data length: ${data.length}`);
    logger.info(`[SessionManager] ==============================`);
    ...
    if (!session && this.useTmux && await this.tmuxSessionExists(sessionId)) {
      ...
      if (this.sessionHistoryManager) {
        ...
        if (dbSession && dbSession.status === 'active') {
          ...
          // Set up handlers
          if (session && session.pty) {
            // Use the existing onData handler passed in config
            session.pty.onData((data: string) => {
              logger.info(`[SessionManager] ====== PTY DATA RECEIVED (writeToSession reattach) ======`);
              logger.info(`[SessionManager] Session: ${sessionId}`);
              logger.info(`[SessionManager] Data length: ${data.length}`);
              logger.info(`[SessionManager] First 50 chars: ${JSON.stringify(data.substring(0, 50))}`);
              logger.info(`[SessionManager] ===============================`);
              ...
              session.lastActivity = new Date();
              this.appendToBuffer(session, filteredData);
              logger.info(`[SessionManager] Emitting sessionData event for writeToSession reattach ${sessionId}, data length: ${filteredData.length}`);
              this.emit('sessionData', { sessionId, data: filteredData });
            ...
            session.pty.onExit(async ({ exitCode }) => {
              logger.info(`Session ${sessionId} exited with code ${exitCode}`);
            ...
            logger.info(`Successfully reattached to session ${sessionId} with handlers`);
            
            // Verify handlers are attached
            const hasHandlers = session.pty && (session.pty as any)._events && (session.pty as any)._events.data;
            logger.info(`[SessionManager] Reattached PTY has onData handlers after setup: ${hasHandlers}`);
          }
    ...
    session.lastActivity = new Date();
    logger.info(`[SessionManager] Writing to PTY: ${JSON.stringify(data)} for session ${sessionId}`);
    logger.info(`[SessionManager] PTY exists: ${!!session.pty}`);
    logger.info(`[SessionManager] PTY pid: ${session.pty?.pid}`);
    
    // Check if this PTY has handlers
    const hasHandlers = session.pty && (session.pty as any)._events && (session.pty as any)._events.data;
    logger.info(`[SessionManager] PTY has onData handlers: ${hasHandlers}`);
    
    try {
      session.pty.write(data);
      logger.info(`[SessionManager] Successfully wrote ${data.length} bytes to PTY`);
    } catch (error: any) {
      logger.error(`[SessionManager] Failed to write to PTY: ${error.message}`);
      logger.error(`[SessionManager] Error stack: ${error.stack}`);
    }
