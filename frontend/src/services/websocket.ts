...
export class WebSocketClient extends EventEmitter {
    ...
    messageTypes.forEach(type => {
      ...
      this.socket!.on(type, (message: ServerToClientMessage) => {
        ...
        if (type === 'terminal_output') {
          ...
          console.log(`[WebSocket] Session ID: ${(message as any).sessionId}`);
          console.log(`[WebSocket] Data length: ${(message as any).data?.length || 0}`);
          console.log(`[WebSocket] First 50 chars: ${(message as any).data?.substring(0, 50)}`);
  ...
  sendTerminalData(sessionId: string, data: string): void {
    console.log(`[WebSocket Client] ====== SENDING TERMINAL DATA ======`);
    console.log(`[WebSocket Client] Session ID: ${sessionId}`);
    console.log(`[WebSocket Client] Data: ${JSON.stringify(data)}`);
    console.log(`[WebSocket Client] Data length: ${data.length}`);
    console.log(`[WebSocket Client] Connected: ${this.socket?.connected}`);
    console.log(`[WebSocket Client] Socket ID: ${this.socket?.id}`);
    console.log(`[WebSocket Client] =================================`);
    
    if (!this.socket?.connected) {
      console.error(`[WebSocket Client] NOT CONNECTED! Cannot send data`);
      return;
    }
